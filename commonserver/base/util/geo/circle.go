...
// IsInside 是否在里面
func (c *Circle) IsInside(coord Coord) bool {
	r := float64(c.Radius)
	return CalDstCoordToCoordWithoutSqrt(c.Center, coord) <= r*r
}
...
// circle: 圆
// vectors: 多边形向量数组，逆时针
// 参考链接：https://slg.bitlush.com/blog/circle-vs-polygon-collision-detection-in-c-sharp
func (c *Circle) IsInterPolygon(vectors []Vector) bool {
	radiusSquared := float64(int64(c.Radius) * int64(c.Radius))

	vertex := vectors[len(vectors)-1] //顶点
