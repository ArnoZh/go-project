...
func GetArcCoords(startCoord, centerCoord Coord, angle float64) []Coord {
	// pi 34个点
	n := util.MaxInt32(int32(util.AbsFloat64(angle)*10), 2)
...
func GetSpiralCoords(startCoord, centerCoord Coord, angle, delta float64) []Coord {
	n := int32(util.AbsFloat64(angle) * 10)
	if n < 2 {
...
// GetBresenhamCoord https://slg.zh.wikipedia.org/wiki/布雷森漢姆直線演算法
func GetBresenhamCoord(p1, p2 Coord) []Coord {
	...
	if util.AbsInt32(zend-zstart) > util.AbsInt32(xend-xstart) {
		steep = true
	}
	...
	var deltay = util.AbsInt32(zend - zstart)
	var err = deltax / 2
	var zstep int32
	z := zstart
	if zstart < zend {
...
func GetIntersectRect(r0 Rectangle, r1 Rectangle) (Rectangle, bool) {
	rect := Rectangle{}
	rect.X = util.MaxInt32(r0.X, r1.X)
	rect.Z = util.MaxInt32(r0.Z, r1.Z)
	rect.Width = util.MinInt32(r0.X+r0.Width, r1.X+r1.Width) - rect.X
	rect.Height = util.MinInt32(r0.Z+r0.Height, r1.Z+r1.Height) - rect.Z
